...
// Constructor
particle::particle(int charge_in, double mass_in, arma::vec position_in, aram::vec velocity_in)
{
  charge_ = charge_in;
  mass_ = mass_in;
...
{
  std::string info_string = std::to_string(charge_) + " " + std::to_string(number_) + " " + std::to_string(position_) + " " + std::to_string(velocity_);
  return info_string;
}