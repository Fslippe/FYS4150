...
\begin{enumerate}[label= \alph*)]
  \item Since the vector $\vec{v*}$ of length $m$ represents a complete solution of the descretized Poisson equation it contains all values in $\vec{v}$ in adition to the boundary conditions. The relation bethween $n$ and $m$ is therfore $m=n+2$.
  \item Solving $\bold{A}\vec{v} = \vec{g}$ for $\vec{v}$ gives us all but the first an last value in $\vec{v*}$. So all but the boundary values.
\end{enumerate}

\section*{Problem 6}
\begin{enumerate}[label= \alph*)]
  \item We can use the Thomas algorithm to row reduce the matrix $A$ to give us a solution of the equation $A \vec{v} = \vec{g}$. This is done in two steps. We first define the three diagonals as three vectors. The sub diagonal $\vec{a}$ the diagonal $\vec{b}$ and the superdiagonal $\vec{c}$ where the $i$ element in theese vectors corresponds to the $i$ row of the matrix $A$.
  \begin{enumerate}[label=\roman*)]
    \item The first step is forwards substitution. We have to define the first index and then loop over the rest:
    \begin{align*}
      \tilde{b}_1 &= b_1 \\
      \tilde{b}_i &= b_i - \frac{a_i}{b_{i -1} }c_{i-1} \quad \text{for }i=2,..,n
    \end{align*}
    \begin{align*}
      \tilde{g}_1 &= g_1 \\
      \tilde{g}_i &= g_i - \frac{a_i}{\tilde{b}_{i -1} }\tilde{g}_{i-1} \quad \text{for }i=2,..,n
    \end{align*}
    ...
    \begin{align*}
      ...
      v_i &= \frac{\tilde{g}_i - c_i v_ {i+1}}{\tilde{b}_i} \quad \text{for } i = n-1,...,1
    \end{align*}
  \end{enumerate}
